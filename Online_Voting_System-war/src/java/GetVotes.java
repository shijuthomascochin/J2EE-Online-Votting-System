 ...
 * @author akhil
 */
@WebServlet(urlPatterns = {"/GetVotes"})
public class GetVotes extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for(Contester cc : c)
                c1.add(cc.getPname()+","+cc.getSchool_name() +","+ cc.getSname()+ "," +cc.getVcount());
            HttpSession session = request.getSession();
            session.setAttribute("cont", c1);
            response.sendRedirect("/Online_Voting_System-war/Admin/ViewVotes.jsp");
        }
    }
